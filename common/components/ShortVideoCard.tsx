...
const ShortVideoCard = (props: Props) => {
  const { data, query, theme = 'home', className = '' } = props
  return (
    <MyLink
      href={{
      ...
      className={
        className ||
        'relative flex shrink-0 flex-col sm:h-[284px] sm:w-full lg:h-[322px] lg:w-full 2xl:h-[352px] 2xl:w-[198px]'
      }
      title={data?.name}
    >
      <ProgressiveImg
      ...
      >
        <div className="mb-2 flex flex-1 flex-wrap font-semibold subtitle">
        ...
        {theme === 'home' ? (
          <span className="text-neutral-400 caption1">
